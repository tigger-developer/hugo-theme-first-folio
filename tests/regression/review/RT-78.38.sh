...
run_test() {
    ...
    mapfile -t game_links < <(htmlq -f "$build_dir/game-reviews/index.html" -a href '.list-view-title a')
    ...
    [[ -n "$(htmlq -f "$build_dir/book-reviews/book-featured/index.html" '.post-featured .review-metadata--book')" ]] || return 1
    [[ -n "$(htmlq -f "$build_dir/game-reviews/signal-at-dusk/index.html" '.post-container.dark-bg .review-metadata--game')" ]] || return 1
    [[ -n "$(htmlq -f "$build_dir/game-reviews/game-hero/index.html" '.post-hero .review-metadata--game')" ]] || return 1
